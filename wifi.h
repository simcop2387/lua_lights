...
#define WLAN_SSID       "Scalar"        // cannot be longer than 32 characters!
...
bool displayConnectionDetails(void)
{
  ...
  {
    LogOut.println(F("Unable to retrieve the IP Address!\r\n"));
    return false;
  }
  else
  {
...
//  LogOut.println(F("\nInitialising the CC3000 ..."));
  ...
  if (!cc3000.begin())
  {
    LogOut.println(F("Unable to initialise the CC3000! Check your wiring?"));
    while(1);
  }
  ...
  if (!cc3000.connectToAP(WLAN_SSID, WLAN_PASS, WLAN_SECURITY)) {
    LogOut.println(F("Failed to connect to AP!"));
    while(1);
  }
